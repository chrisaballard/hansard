...
class HansardFileParser:
    def __init__(
        self, 
        entity_labels: List[str]
    ):
        self._entity_detector = EntityDetector(entity_labels)

    def extract_division_votes(self, division: ET.Element) -> List[PersonVote]:
        votes = []
    ...
    def extract_paragraph_text_entities(self, paragraph: ET.Element) -> List[str]:
    ...
    def parse_xml_file(
        ...
        for child in root:
            if child.tag == TagType.MAJOR_HEADING.value:
                ...
                text = [self.extract_paragraph_text_entities(p) for p in child.findall("p")]
                speech = Speech(id=child.attrib["id"], type=speech_type, person=person, text=text)

                # Detect entities in speech
                speech = self._entity_detector.get_speech_entities(speech)
                ...
                # Parse division
                division = self.extract_division_votes(child)
                if current_minor_heading:
